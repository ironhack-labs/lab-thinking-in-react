...
class FilterableProductTable extends Component {
    ...
    render() {
        ...
        const products = filtered.map(product => {
            ...
            return (
                <>
                    <h1 className="Page-title"> Iron Store</h1>
                        <SearchBar inStock={this.state.inStock} query={this.state.query} search={this.handleSearch} products={this.props.products} handleInStock={this.handleInStock}/>
                        <ProductTable products={products} filtered={filtered}/> 
                </>
        )
    }
}
